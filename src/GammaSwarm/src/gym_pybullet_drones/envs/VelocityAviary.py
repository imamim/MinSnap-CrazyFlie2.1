...
class VelocityAviary(BaseAviary):
    ...
    def __init__(self,
        ...
        if drone_model in [DroneModel.CF2X.value, DroneModel.CF2P.value]:
    ...
    def _preprocessAction(self,
        ...
        for k, v in action.items():
            ...
            temp, _, _ = self.ctrl[int(k)].computeControl(control_timestep=self.AGGR_PHY_STEPS*self.TIMESTEP, 
                                                    ...
                                                    target_vel=self.SPEED_LIMIT * np.abs(v[0:3]) * v_unit_vector, # target the desired velocity vector
                                                    target_rpy_rates = (target_angular_rate[k])
                                                    )
            rpm[int(k),:] = temp
        return rpm
